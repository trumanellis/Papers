...
\textcolor{red}{
A robust DPG method is one in which the norm of convergence is both equivalent to $L^2$ and for which the optimal test functions
do not contain boundary layers.
Most of the theory of DPG falls apart when the optimal test functions contain boundary layers as they would remain unresolved for any practical method.
}
...
but DPG uses multi-scale basis functions not to approximate the solution, 
but to approximate optimal test functions in the sense of Barrett and Morton (see \cite{LocalGlobalDPG}).
For each basis function, these test functions are defined as solutions of the global adjoint equation 
with the basis function as the forcing term, and it can be shown that the resulting test space 
delivers $L^2$-optimal solutions.  
...
In a similar sense, DPG can also be interpreted as a Variational Multiscale Method \cite{ChanEvansQiu2014}
