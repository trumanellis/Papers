...
\item Define $w=e^{-t}v$ and note that $\pd{w}{t}=\LRp{\pd{v}{t}-v}e^{-t}$ while $\Grad w=\cancelto{0}{\Grad e^{-t}} v+e^{-t}\Grad v$ and
...
\begin{equation*}
-\tilde\bfbeta\cdot\Gradxt(w)-\epsilon\Delta w=u
\end{equation*}
or 
\begin{equation*}
\tilde\bfbeta\cdot\Gradxt(v)-v+\epsilon\Delta v=-e^{t}u
\end{equation*}
Multiply by $-v$ and integrate to get
\begin{align*}
\int_Q-\tilde\bfbeta\cdot\Gradxt vv+v^2-\epsilon\Delta vv=\int_Qe^{t}uv
\end{align*}
Then
\begin{align*}
\norm{v}^2&=\int_Qe^{t}uv
...
Or
\begin{align*}
\norm{v}^2+\epsilon\norm{\Grad v}^2&=\int_Qe^{t}uv
-\frac{1}{2}\int_Q\underbrace{\Divxt\tilde\bfbeta}_{=0}(v)^2
...
\\
&\leq\int_Qe^{t}\int_Quv
\\
&\leq\int_Qe^{t}\LRp{\frac{\norm{u}^2}{2}+\frac{\norm{v}^2}{2}}
\\
\end{align*}
% \item Since $\Curl\bfbeta=0$ and $\Omega$ is simply connected, there exists a scalar potential $\psi$, $\Grad\psi=\bfbeta$.
% The potential is non-unique up to a constant, and we choose the constant such that $\exp{\psi}=O(1)$. 
% Take the transformed function $w=\exp{\psi}v$ and substitute it into the left hand side of the adjoint equation.
% \[
% -\tilde\bfbeta\cdot\Gradxt w
% \]
\end{enumerate}
