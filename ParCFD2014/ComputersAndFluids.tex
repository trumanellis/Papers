...
Preliminary spatially 1D results for the Sod shock tube and Noh implosion problems are extremely promising.
...
and a flux unknown (from the conservation law), but Demkowicz and Gopalakrishnan also formulated a \emph{primal DPG} method\cite{PrimalDPG} for second order equations
...
\end{equation}
where
$\bftau$ is in a broken tensor product space of $\bfH(\text{div})$ spatially and $L^2$ temporally, while 
...
% The details are complicated and discussed in \cite{ChanHeuerThanhDemkowicz2012} and an upcoming paper on robust norms for transient convection-diffusion,
% but this norm is ultimately derived from the adjoint equation for convection-diffusion:
% \begin{align*}
% \frac{1}{\epsilon}\bftau+\Grad v &=\bff\\
% -\tilde\bfbeta\cdot\Gradxt v + \Div\bftau &= g\,,
% \end{align*}
% where the first equation corresponds to the constitutive equation and the second to the conservation law.
...
% \end{aligned}
% \end{equation}
% We now explain how we derived the analogous norm for Navier-Stokes. 
% The first term in \eqref{eq:robustConfusion}, $\frac{1}{\epsilon}\norm{\bftau}^2$, becomes 
% $\frac{1}{\mu}\norm{\mathbb{S}}^2
% +\frac{Pr}{C_p\mu}\norm{\bftau}^2$.
% The fourth term in \eqref{eq:robustConfusion}, $\epsilon\norm{\Grad v}^2$ comes from rescaling the leftover part of the constitutive adjoint by viscosity
% with analogous terms 
% $
% \mu\norm{\Grad\bfv_m+\Grad v_e\otimes\tilde\bfu}^2
% +\frac{C_p\mu}{Pr}\norm{\Grad v_e}^2
% $
% for Navier-Stokes.
% The second term in \eqref{eq:robustConfusion} corresponds to the entire adjoint for the conservation law in convection-diffusion, 
% thus we insert the analogous terms directly from the graph norm. These are the next three terms in the Navier-Stokes norm.
% The third term in \eqref{eq:robustConfusion} comes from the adjoint to the convective part of the equation. 
% By analogy, we take the adjoint to the Euler equations, producing the next three terms in \eqref{eq:robustNS}.
% The first of these three is identical to the first of the previous group since there are no viscous terms in the conservation of mass equation.
% The last three $L^2$ terms in \eqref{eq:robustNS} correspond to the final $L^2$ term in \eqref{eq:robustConfusion}.
...
(development of robust norms for compressible Navier-Stokes will be the topid of an upcoming paper).
...
Again, unnecessary refinements show up, but in this case, we believe they are more of a response to the odd shock behavior on mid-resolution meshes.
...
We've presented the initial developments of a space-time DPG method for parabolic PDEs with automatic adaptivity in space and time.
