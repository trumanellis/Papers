...
Roberts \etal present a brief history and derivation of DPG with optimal test functions in
\cite{DPGStokes}. We follow their derivation of the standard DPG method as a
...
% Of course, \eqnref{optimaltestproblem} still requires the inversion of the
...
% ``enriched space'') works well in practice\red{\cite{DPGOverview2}}.
...
In this contribution, we assume that the error resulting from the approximation of test functions
...
(a robust technique remains stable as the singularly perturbed parameter approaches ``extreme'' values).
...
acoustics \cite{DemkowiczGopalakrishnanMugaZitelli12}, and Stokes
...
can expect that the $L^2$ error will also decrease uniformly in $\epsilon$.
...
$$
In the context of the DPG method, this translates in a simple condition that test function
...
% \red{Element conservation is defined such that the surface integral of fluxes is balanced by the volume integral of the source terms element by element.}
% \red{Element conservation is defined such that the balance of fluxes and source terms is satisfied: $\int_{\partial K}\hat t=\int_K f$.}
...
We follow the classical Brezzi theory \cite{Brezzi1974,DBB05} for an abstract
mixed problem:
\begin{equation}
\left\{
\begin{array}{lll}
...
Classical $h$-interpolation error estimates for the lowest error
...
\label{eq:stab}
\end{equation}
Above, $C$ is a generic, mesh independent constant incorporating the constant from
...
Notice that we have considered traces of the lowest order Raviart-Thomas elements
...
% \red{With a locally conservative formulation, we can take $\alpha=0$ in local problem \eqref{eq:localSolve}. 
...
% next section.
% It is convenient to be able to take \red{$\alpha=0$} as we will see in
% some later numerical experiments.
% \red{We've noticed that this particularly helps with conditioning of the local problem as the mesh size decreases.}
...
of boundary edges to 1 at corners. We do not enforce an outflow boundary.
