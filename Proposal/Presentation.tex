...
{\footnotesize Advisors: Leszek Demkowicz, Robert Moser}
}
...
\framesubtitle{Two Views: Residual Based Stabilization}
% SUPG pioneered the idea of modifying the test space to improve stability\footfullcite{SUPG}.

Convection-diffusion can be written as
$
Lu=(L_{adv}+L_{diff})u=f\,.
$\\
The SUPG\footfullcite{SUPG}
problem is $b_{SUPG}(u,v)=l_{SUPG}(v)$ where
\begin{align*}
  b_{SUPG}(u,v)&=b(u,v)+\sum_K\int_K\tau(L_{adv}v)(Lu-f)\\
  l_{SUPG}(v)&=l(v)+\sum_K\int_K\tau(L_{adv}v)f\,,
\end{align*}
and $\tau$ is the SUPG stabilization parameter.
\begin{itemize}
...
\footnote{SUPG spawned a huge interest in stabilized methods. A small sampling of contributors includes L.P. Franca, C. Johnson, R. Codina, T. Tezduyar and many others. The variational multiscale method has become a prominent heir to and generalization of SUPG.} 
\end{itemize}
\end{frame}
...
\frametitle{Streamline Upwind Petrov-Galerkin}
\framesubtitle{Two Views: Stabilization by Modifying the Test Space}
% SUPG pioneered the idea of modifying the test space to improve stability\footfullcite{SUPG}.
System can alternatively be written as
\[
b(u,\tilde v)=l(\tilde v)
\]
where
...
\begin{figure}[t]
\centering
\includegraphics[height=0.3\textheight]{Motivation/SUPGtest.png}
\\\small{Upwind Bias of SUPG Test Function}\\
\end{figure}
\end{column}
\begin{column}{.5\textwidth}
\begin{figure}[t]
\centering
\includegraphics[height=0.3\textheight]{Motivation/SUPG.png}
\\\small{$H_0^1$ Projection of SUPG Solution}\\
\end{figure}
\end{column}
\end{columns}
\end{frame}
...
\]
\begin{itemize}
  \item Hybridized DPG method introduces trace unknowns but allows static condensation\footfullcite{HDG}
\end{itemize}
\end{frame}
...
\begin{itemize}
  ...
  \item Satisfies geometric conservation laws\footfullcite{GCL}
  \item Tezduyar \etal\footfullcite{Tezduyar1992} developed a Galerkin/least-squares method to handle moving boundaries
  ...
  \item Space-time DG\footfullcite{vanderVegtEuler} has become a popular choice
  \item Tent-pitcher algorithm\footfullcite{TentPitcher} decouples space-time elements
\end{itemize}
\end{frame}
...
Costabel and McIntosh\textsuperscript{\footfullcite{CostabelMcIntosh}}.
...
\end{array}
\end{equation*}
Let $\hat{t}$ be the trace of $\bfpsi_h$, then
\begin{align*}
