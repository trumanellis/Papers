...
\framesubtitle{Superior Stability Simplifies Mesh Generation}  %% needed for proper positioning of the logo ...
\begin{itemize}
  \item Initial mesh design is a time-consuming and expensive part of CFD
  \item DPG has no pre-asymptotic regime
  \item Works on the coarsest possible mesh which represents the geometry
  \item DPG + NURBS
\end{itemize}
\end{frame}
...
\begin{figure}[t]
\centering
\includegraphics[width=1.0\textwidth]{Motivation/PointwiseAirfoil1.png}\\
\includegraphics[width=1.0\textwidth]{Motivation/PointwiseAirfoil2.png}
\\\small{Airfoil mesh by Pointwise}\\
\end{figure}
\end{column}
\end{columns}
\end{frame}
